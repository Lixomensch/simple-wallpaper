...
pub fn generate_or_get_thumbnail(
    ...
    let cache_path = thumbnail_path_for(original_path, max_width, max_height, &cache_dir);
    ...
    load_thumbnail_data(&cache_path)
}
...
fn thumbnails_dir() -> Result<PathBuf, SwpError> {
    ...
    fs::create_dir_all(&dir).map_err(|e| SwpError::Thumbnail {
        message: format!("Failed to create thumbnails directory {}: {e}", dir.display()),
    })?;
    Ok(dir)
}
...
fn thumbnail_path_for(
    ...
    cache_dir.join(format!("{:x}_{}x{}.png", digest, max_width, max_height))
}
